 ...
 */
const defaultIsAssetManifestIcon = (fileName) => !!fileName.match(/manifest\/icon_(\d+)x(\d+)-\w*\.(png|jpeg|jpg)$/);
/**
...
const defaultGetIconSize = (fileName) => {
    ...
    if (!width || Number.isNaN(width)) {
        throw new Error(`Invalid icon width found ${JSON.stringify(width)} in filename ${JSON.stringify(fileName)}`);
    }
    const height = match[2] && parseInt(match[2], 10);
    if (!height || Number.isNaN(height)) {
        throw new Error(`Invalid icon height found ${JSON.stringify(height)} in filename ${JSON.stringify(fileName)}`);
    }
    return { width, height };
};
/**
...
module.exports = class WebAppManifestPlugin {
     ...
     */
    constructor({ content, destination, useDigest = true, isAssetManifestIcon = defaultIsAssetManifestIcon, getIconSize = defaultGetIconSize, getIconType = defaultGetIconType, }) {
        ...
        compiler.hooks.thisCompilation.tap(pluginName, (compilation) => {
            compilation.hooks.processAssets.tap({ name: pluginName, stage: Compilation.PROCESS_ASSETS_STAGE_SUMMARIZE }, (assets) => {
                ...
                */
                ...
                const digest = hash.update(content).digest('hex').substring(0, 8);
